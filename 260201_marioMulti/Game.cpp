...
// Constants matching centralized version
const char BRICK = '#';
...
void Game::initializeLevel(int level) {
    system("color 9F"); // Match centralized version's color setting
...
void Game::update() {
    // Update Mario (vertical physics and collision)
    moveObjectVertically(mario);

    // If Mario fell below map, restart level (dead)
    ...
    // Handle collisions between Mario and movings
    marioCollision();
}
...
void Game::moveObjectVertically(Object& obj) {
    ...
    for (int i = 0; i < bricks_count; i++) {
        if (isCollision(obj, bricks[i])) {
            ...
            if (prevSpeed > 0) {
                obj.is_fly = false;
                // snap Mario (or object) to top of the brick to avoid sub-pixel jitter
                obj.y = bricks[i].y - obj.height;
            }

            // If Mario hits a FULL_BOX from below, spawn money
            ...
            // If hit from below, push object just below the brick to avoid overlap
            if (prevSpeed < 0) {
        ...
        if (isCollision(mario, movings[i])) {
            if (movings[i].ctype == ENEMY) {
                if (mario.is_fly && mario.vert_speed > 0 &&
                    ...
                    return; // level reinitialized, stop processing
                }
            }
...
void Game::moveMapHorizontally(float dx) {
    // Attempt to shift bricks and movings by dx; if Mario collides after shift, revert
    for (int i = 0; i < bricks_count; i++) bricks[i].x += dx;
    for (int i = 0; i < movings_count; i++) movings[i].x += dx;

    // If Mario collides with any brick after shift, revert and do nothing
    for (int i = 0; i < bricks_count; i++) {
